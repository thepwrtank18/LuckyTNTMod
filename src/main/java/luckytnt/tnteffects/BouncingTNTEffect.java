...
public class BouncingTNTEffect extends PrimedTNTEffect{
	...
	public void explosionTick(IExplosiveEntity entity) {
		...
		if(((Entity)entity).getPersistentData().getInt("bounces") >= 12) {
			entity.level().playSound(null, new BlockPos(entity.getPos()), SoundEvents.GENERIC_EXPLODE, SoundSource.BLOCKS, 4f, (1f + (entity.level().getRandom().nextFloat() - entity.level().getRandom().nextFloat()) * 0.2f) * 0.7f);
