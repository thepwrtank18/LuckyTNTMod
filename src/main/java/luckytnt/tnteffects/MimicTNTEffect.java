...
public class MimicTNTEffect extends PrimedTNTEffect {
	...
	public void explosionTick(IExplosiveEntity ent) {
		...
		if (ent.level().getNearestPlayer((Entity) ent, 5) != null && ent.level().getNearestPlayer((Entity) ent, 5) != ent.owner()) {
			ent.level().playSound(null, new BlockPos(ent.getPos()), SoundEvents.GENERIC_EXPLODE, SoundSource.BLOCKS, 4f, (1f + (ent.level().getRandom().nextFloat() - ent.level().getRandom().nextFloat()) * 0.2f) * 0.7f);
